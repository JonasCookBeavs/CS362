...
# Added LoggingEmail class with inheritence
class LoggingEmail < Email
  ...
  # Overwrote send method
  def send(to)
    puts("#{@email.headers[:from]}, #{to}, #{@email.header[:subject]}")
    @email.send()
  end
...
lm = LoggingEmail.new(m3)

m3.send("leslie@example.com")
lm.send("leslie@example.com")